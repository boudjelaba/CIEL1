...
//inclure la page de connexion
 ...
 if(!isset($_SESSION['panier'])){
    //s'il n'existe pas une session, on créer une et on met un tableau a l'intérieur 
    $_SESSION['panier'] = array();
 }
 //récupération de l'id dans le lien
  if(isset($_GET['id_prod'])){//si un id a été envoyé alors :
    $id = $_GET['id_prod'];
    //verifier grace a l'id si le produit existe dans la base de  données
    $produit = mysqli_query($con ,"SELECT * FROM produits WHERE id_prod = $id");
